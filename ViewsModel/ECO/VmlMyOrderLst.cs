...
{
    public class VmlMyOrderLst : BaseViewModel
    {
        #region "Declaring"
        string mRequest = "";
        string mResponse = "";
        
        public JSN_REQ_SALE_ORDER_JUN mJSN_REQ_SALE_ORDER_JUN = new JSN_REQ_SALE_ORDER_JUN();
        public JSN_SALE_ORDER_JUN mJSN_SALE_ORDER_JUN = new JSN_SALE_ORDER_JUN();
        public JSN_LOAD_SALE_INVOICE mJSN_LOAD_SALE_INVOICE = new JSN_LOAD_SALE_INVOICE();
        public List<RES_SALE_ORDER> mRES_SALE_ORDER_LST = new List<RES_SALE_ORDER>();
        public ObservableCollection<RES_SALE_ORDER> SaleOrderLst { get; set; }
        public ObservableCollection<SortingItem> sortingList { get; set; }
        SortingItem[] labelTexts = [
            new SortingItem{ label = Common.mCommon.GetLanguageValueByKey("ECO.MyOrder.lbl.Code"), value = "OrderCode_0_50", ShowIcon = true },
            new SortingItem{ label = Common.mCommon.GetLanguageValueByKey("ECO.MyOrder.lbl.Date"), value = "OrderDate", ShowIcon = false },
            new SortingItem{ label = Common.mCommon.GetLanguageValueByKey("ECO.MyOrder.lbl.SalesPerson"), value = "SalePersonName_0_255", ShowIcon = false },
            new SortingItem{ label = Common.mCommon.GetLanguageValueByKey("ECO.MyOrder.lbl.Status"), value = "StatusName_0_255", ShowIcon = false },
            new SortingItem{ label = Common.mCommon.GetLanguageValueByKey("ECO.MyOrder.lbl.Total"), value = "GrandTotal", ShowIcon = false}
            ];

        #endregion

        #region "Contructor"
        public VmlMyOrderLst()
        {
            this.switchDisplayView(DisplayView.Card);
            SalesInvoiceLoad = new JSN_LOAD_SALE_INVOICE();
            SaleOrderLst = new ObservableCollection<RES_SALE_ORDER>();
            ...
            get
            {
                if (mRefreshCommand == null)
                {
                    mRefreshCommand = new Command(() => {
                        ...
                        //    this.getInvoice();
                        //}
                        mJSN_REQ_SALE_ORDER_JUN.RES_SALE_ORDER = new RES_SALE_ORDER();
                        mJSN_REQ_SALE_ORDER_JUN.RES_SALE_ORDER.Sequence = "0";
                        this.getMyOrder();
                    });
                }
        ...
        public ICommand DeleteItemCommand
        {
            get
            {
                ...
                {
                    mDeleteItemCommand = new Command<RES_SALE_ORDER>(async (item) =>
                    {
                        if (Utility.checkButtonAccess("Delete") && item.PostingStatusAsk != "1" && item.StatusAsk != "9")
                        {
                            ...
                            if (answer)
                            {
                                mJSN_REQ_SALE_ORDER_JUN.RES_SALE_ORDER = item;
                                mJSN_REQ_SALE_ORDER_JUN.RES_SALE_ORDER.StatusAsk = "6";
                                saveInvoice();
                            }
                        }
                        else
        ...
        public ICommand ActiveItemCommand
        {
            ...
            {               
                ...
                {
                    mActiveItemCommand = new Command<RES_SALE_ORDER>(async (item) =>
                    {
        ...
        private void switchDisplayView(DisplayView argDisplayView)
        {
            try
            {
                ...
                var tmp = SaleOrderLst;
                SaleOrderLst = null;
                NotifyPropertyChanged(nameof(SaleOrderLst));
        ...
        private void bindDataTab(List<RES_SALE_ORDER> argRES_SALE_ORDER)
        {
            try
            {
                if (argRES_SALE_ORDER != null && argRES_SALE_ORDER.Count > 0)
                {
                    foreach (RES_SALE_ORDER l_RES_SALE_ORDER in argRES_SALE_ORDER)
                    {
                        SaleOrderLst.Add(l_RES_SALE_ORDER);
                    }
                }
                else
                {
        ...
        public void searchDataApi(string argKeyword)
        {
            ...
            {
                mJSN_REQ_SALE_ORDER_JUN.RES_SALE_ORDER = new RES_SALE_ORDER();
                mJSN_REQ_SALE_ORDER_JUN.RES_SALE_ORDER.Remark = argKeyword;
                getMyOrder();
            }
            catch (Exception ex)
            {
        ...
        public void searchData(string argKeyword)
        {
            try
            {
                ...
                {
                    foreach (RES_SALE_ORDER l_RES_SALE_ORDER in mJSN_SALE_ORDER_JUN.RES_SALE_ORDER)
                    {
                        ...
                        if (l_RES_SALE_ORDER.OrderCode_0_50.ToLower().Contains(argKeyword)
                            ...
                            || l_RES_SALE_ORDER.OutstandingAmount.ToLower().Contains(argKeyword)
                ...
                {
                    l_RES_SALE_ORDER_Lst = new List<RES_SALE_ORDER>(mJSN_SALE_ORDER_JUN.RES_SALE_ORDER);// OriginalInvoiceClosedList.GetRange(0, OriginalInvoiceClosedList.Count);
                }
        ...
        private async void callSearchMorePopup()
        {
            try
            {
                ...
                {
                    mJSN_REQ_SALE_ORDER_JUN.RES_SALE_ORDER = selectedData;
                    if(Common.mCommon.UserSetting.TLSearchTypeAsk == "1")//1 for local search
                    {
        ...
        public async void getMyOrder()
        {
            ...
            {
                Utility.openLoader();
                mRequest = JsonConvert.SerializeObject(mJSN_REQ_SALE_ORDER_JUN);
                mResponse = await Eco_Service.ApiCall(mRequest, Eco_Name.wsgetMyOrder);
                if (mResponse != null && mResponse != "")
                {
                    this.mJSN_SALE_ORDER_JUN = JsonConvert.DeserializeObject<JSN_SALE_ORDER_JUN>(mResponse);
                    if (this.mJSN_SALE_ORDER_JUN.Message.Code == "7")
                    {
                        if (this.mJSN_SALE_ORDER_JUN.RES_SALE_ORDER.Count > 0)
                        {
                            mRES_SALE_ORDER_LST = this.mJSN_SALE_ORDER_JUN.RES_SALE_ORDER;
                            bindDataTab(this.mJSN_SALE_ORDER_JUN.RES_SALE_ORDER);
                            WeakReferenceMessenger.Default.Send(this.mJSN_SALE_ORDER_JUN.Message.Message);
                        }
                        else
                        {
                            WeakReferenceMessenger.Default.Send(this.mJSN_SALE_ORDER_JUN.Message.Message);
        ...
        public async void saveInvoice()
        {
            try
            {
                ...
                //    {
                //        mJSN_REQ_WISHLIST.RES_SALE_ORDER = new RES_SALE_ORDER();
                //        getWishlist();
                //    }
                //    else
                //    {
