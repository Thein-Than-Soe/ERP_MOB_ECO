...
{
    public class VmlWishlist : BaseViewModel
    {
        #region "Declaring"
        string mRequest = "";
        string mResponse = "";
        
        public JSN_REQ_WISHLIST mJSN_REQ_WISHLIST = new JSN_REQ_WISHLIST();
        public JSN_RES_WISHLIST mJSN_RES_WISHLIST = new JSN_RES_WISHLIST();
        public JSN_LOAD_SALE_INVOICE mJSN_LOAD_SALE_INVOICE = new JSN_LOAD_SALE_INVOICE();
        public List<DAT_WISHLIST> mDAT_WISHLIST_LST = new List<DAT_WISHLIST>();
        public ObservableCollection<DAT_WISHLIST> WishlistLst { get; set; }
        public ObservableCollection<SortingItem> sortingList { get; set; }
        SortingItem[] labelTexts = [
            new SortingItem{ label = Common.mCommon.GetLanguageValueByKey("ECO.MyWishlist.lbl.Code"), value = "WishlistCode_0_50", ShowIcon = true },
            new SortingItem{ label = Common.mCommon.GetLanguageValueByKey("ECO.MyWishlist.lbl.Date"), value = "WishlistDate", ShowIcon = false },
            new SortingItem{ label = Common.mCommon.GetLanguageValueByKey("ECO.MyWishlist.lbl.SalesPerson"), value = "SalePersonName_0_255", ShowIcon = false },
            new SortingItem{ label = Common.mCommon.GetLanguageValueByKey("ECO.MyWishlist.lbl.Status"), value = "StatusName_0_255", ShowIcon = false },
            new SortingItem{ label = Common.mCommon.GetLanguageValueByKey("ECO.MyWishlist.lbl.Total"), value = "GrandTotal", ShowIcon = false} 
            ];
        ...
        public VmlWishlist()
        {
            ...
            WishlistLst = new ObservableCollection<DAT_WISHLIST>();
            ...
            get
            {
                if (mRefreshCommand == null)
                {
                    mRefreshCommand = new Command(() => {
                        ...
                        mJSN_REQ_WISHLIST.DAT_WISHLIST.Sequence = "0";
                        this.getWishlist();
                    });
                }
        ...
        public ICommand DeleteItemCommand
        {
            get
            {
                ...
                {
                    mDeleteItemCommand = new Command<DAT_WISHLIST>(async (item) =>
                    {
                        if (Utility.checkButtonAccess("Delete") && item.PostingStatusAsk != "1" && item.StatusAsk != "9")
                        {
                            bool answer = await Application.Current.MainPage.DisplayAlert(
                               $"{item.WishlistCode_0_50}",
                            ...
                            if (answer)
                            {
                                mJSN_REQ_WISHLIST.DAT_WISHLIST = item;
                                mJSN_REQ_WISHLIST.DAT_WISHLIST.StatusAsk = "6";
                                saveInvoice();
                            }
                        }
                        else
        ...
        public ICommand ActiveItemCommand
        {
            ...
            {               
                ...
                {
                    mActiveItemCommand = new Command<DAT_WISHLIST>(async (item) =>
                    {
        ...
        private void switchDisplayView(DisplayView argDisplayView)
        {
            try
            {
                ...
                var tmp = WishlistLst;
                WishlistLst = null;
                NotifyPropertyChanged(nameof(WishlistLst));
        ...
        private void bindDataTab(List<DAT_WISHLIST> argDAT_WISHLIST)
        {
            try
            {
                if (argDAT_WISHLIST != null && argDAT_WISHLIST.Count > 0)
                {
                    foreach (DAT_WISHLIST l_DAT_WISHLIST in argDAT_WISHLIST)
                    {
                        WishlistLst.Add(l_DAT_WISHLIST);
                    }
                }
                else
                {
        ...
        public void searchDataApi(string argKeyword)
        {
            ...
            {
                mJSN_REQ_WISHLIST.DAT_WISHLIST = new DAT_WISHLIST();
                mJSN_REQ_WISHLIST.DAT_WISHLIST.Remark = argKeyword;
                getWishlist();
            }
            catch (Exception ex)
            {
        ...
        public void searchData(string argKeyword)
        {
            try
            {
                ...
                {
                    foreach (DAT_WISHLIST l_DAT_WISHLIST in mJSN_RES_WISHLIST.DAT_WISHLIST)
                    {
                        argKeyword = argKeyword.ToLower();
                        if (l_DAT_WISHLIST.WishlistCode_0_50.ToLower().Contains(argKeyword)
                            || l_DAT_WISHLIST.WishlistDate.ToLower().Contains(argKeyword)
                            || l_DAT_WISHLIST.OutstandingAmount.ToLower().Contains(argKeyword)
                            || l_DAT_WISHLIST.GrandTotal.ToLower().Contains(argKeyword))
                        {
                            l_DAT_WISHLIST_Lst.Add(l_DAT_WISHLIST);
                        }
                    }
                }
                else
                {
                    l_DAT_WISHLIST_Lst = new List<DAT_WISHLIST>(mJSN_RES_WISHLIST.DAT_WISHLIST);// OriginalInvoiceClosedList.GetRange(0, OriginalInvoiceClosedList.Count);
                }
                bindDataTab(l_DAT_WISHLIST_Lst);
            }
            catch (Exception ex)
            {
                throw ex.InnerException;
        ...
        private async void callSearchMorePopup()
        {
            try
            {
                ...
                if (result is DAT_WISHLIST selectedData)
                {
                    mJSN_REQ_WISHLIST.DAT_WISHLIST = selectedData;
                    if(Common.mCommon.UserSetting.TLSearchTypeAsk == "1")//1 for local search
                    {
                        WishlistLst = new ObservableCollection<DAT_WISHLIST>(mDAT_WISHLIST_LST.Where(data =>(data.CustomerAsk == selectedData.CustomerAsk)
                                                                               || (data.WishlistCode_0_50 == selectedData.WishlistCode_0_50)).ToList());
                    }
                    else
                    {
                        getWishlist();
        ...
        public async void getWishlist()
        {
            try
            {
                ...
                mResponse = await Eco_Service.ApiCall(mRequest, Eco_Name.wsgetMyWishlist);
                if (mResponse != null && mResponse != "")
                {
                    ...
                    {
                        if (this.mJSN_RES_WISHLIST.DAT_WISHLIST.Count > 0)
                        {
                            mDAT_WISHLIST_LST = this.mJSN_RES_WISHLIST.DAT_WISHLIST;
                            bindDataTab(this.mJSN_RES_WISHLIST.DAT_WISHLIST);
